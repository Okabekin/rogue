...
public class InventoryManager : MonoBehaviour
{
    ...
    public void LevelUpWeapon(int slotIndex, int upgradeIndex)
    {
        if(weaponSlots.Count > slotIndex)
        {
            ...
            weaponLevels[slotIndex] = upgradedWeapon.GetComponent<WeaponController>().weaponData.Level; //to make sure we have correct weapon level
    ...
    public void LevelUpPassiveItem(int slotIndex, int upgradeIndex)
    {
        if (passiveItemSlots.Count > slotIndex)
        {
            ...
            passiveItemLevels[slotIndex] = upgradedPassiveItem.GetComponent<PassiveItem>().passiveItemData.Level; // to make sure we have correct weapon level
        ...
        foreach(var upgradeOption in upgradeUIOptions)
        {
                    ...
                    for (int i = 0; i < weaponSlots.Count; i++)
                    {
                        if (weaponSlots[i] != null && weaponSlots[i].weaponData == chosenWeaponUpgrade.weaponData)
                        {
                            ...
                            if(!newWeapon)
                            {
                                ...
                                //set description and name to be that of next level
            ...
            else if (upgradeType ==2)
            {
                ...
                if(chosenPassiveItemUpgrade != null)
                {
                    ...
                    for ( int i = 0; i < passiveItemSlots.Count; i ++)
                    {
                        if (passiveItemSlots[i] != null && passiveItemSlots[i].passiveItemData == chosenPassiveItemUpgrade.passiveItemData)
                        {
                            ...
                            if (!newPassiveItem)
                            {
                                ...
                                upgradeOption.upgradeButton.onClick.AddListener(() => LevelUpPassiveItem(i, chosenPassiveItemUpgrade.passiveItemUpgradeIndex)); // appl button funcitonality
                                //set description and name to be that of next level
                    ...
                    if (newPassiveItem)
                    {
                        upgradeOption.upgradeButton.onClick.AddListener(() => player.SpawnPassiveItem(chosenPassiveItemUpgrade.initialPassiveItem)); // applies button functionality
                        //apply initial description and name
